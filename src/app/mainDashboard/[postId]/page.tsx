...
useEffect(()=>{
 ; (async()=>{
    const res = await axios.post("/api/users/comments/getcomments", {postId})
    ...
    <div className="h-[100vh] overflow-hidden">
      <nav className="bg-white shadow-md ">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          {token ? <div className="flex flex-col items-center justify-between h-10">
          ...
          </div> : <button onClick={(e)=>registerHandler(e)} className=" p-2 border border-gray-500  rounded-lg hover:text-white hover:bg-slate-700 hover:shadow-lg transition duration-300">Register</button>}
        </div>
      </nav>
      ...
      <div className="container  mx-auto px-4 py-6 flex-grow  ">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 ">
          {/* <!-- Sidebar --> */}
          {token ? <aside className="md:col-span-1 bg-white p-4 rounded-lg shadow-md ">
            ...
            >
              {SinglePostData.map((post) => (
                <div className="border-b cursor-pointer border-gray-200 p-4 shadow-lg rounded-lg h-full bg-gray-400">
                  ...
                  <p className="text-gray-700 h-[10%] ">{post.content.message}</p>
                  <div className="w-full h-[10%]">
                    <span className="flex gap-4">
                     
                    <div className="flex flex-col items-center">
                    <i onClick={()=>{likeHandler(postId); }} className={likedData.some((obj) => obj.postId === post._id)? "fa-solid fa-heart": "fa-regular fa-heart" }></i> <div className="text-xs font-light">{postLikeCounts[postId] || 0}</div>
                    </div>

                    <div className="flex flex-col items-center">
                    <i onClick={() => {commentHandler();}}
                        className="fa-regular fa-comment"
                      ></i><div className="text-xs font-light">{postCommentCounts[postId] || 0}</div>
                    </div>
                      

                      <i onClick={shareHandler} className="fa-regular fa-share-from-square"></i>
                    </span>
                  </div>
                  ...
                  {contextData.value.commentVisibleId ? (
                    <div className="h-[60%]">
                      <div className=" h-[70%] px-2 overflow-scroll no-scrollbar">
                        {commentData.map((data)=><div className="flex mt-2 flex-col p-2 rounded-lg bg-slate-300">
